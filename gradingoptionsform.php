...
/**
 * edusignment grading options form
 *
 * @package   mod_edusign
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class mod_edusign_grading_options_form extends moodleform
{
    ...
    public function definition()
    {
        ...
        if (!empty($instance['markingallocationopt'])) {
            $markingfilter = get_string('markerfilter', 'edusign');
        ...
        if (!empty($instance['markingworkflowopt'])) {
            $workflowfilter = get_string('workflowfilter', 'edusign');
            $mform->addElement('select', 'workflowfilter', $workflowfilter, $instance['markingworkflowopt'], $dirtyclass);
        }
        // Quickgrading.
        if ($instance['showquickgrading']) {
            $mform->addElement('checkbox', 'quickgrading', get_string('quickgrading', 'edusign'), '', $dirtyclass);
            $mform->addHelpButton('quickgrading', 'quickgrading', 'edusign');
        ...
        if ($instance['submissionsenabled']) {
            $mform->addElement('checkbox', 'downloadasfolders', get_string('downloadasfolders', 'edusign'), '', $dirtyclass);
            $mform->addHelpButton('downloadasfolders', 'downloadasfolders', 'edusign');
