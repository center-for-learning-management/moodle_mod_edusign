...
/**
 * edusignment grade form
 *
 * @package   mod_edusign
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_edusign_grade_form extends moodleform {
    /** @var edusignment $edusignment */
    private $edusignment;
    ...
    public function definition() {
        ...
        list($edusignment, $data, $params) = $this->_customdata;
        // Visible elements.
        $this->edusignment = $edusignment;
        $edusignment->add_grade_form_elements($mform, $data, $params);
    ...
    public function validation($data, $files) {
        ...
        if ($instance->grade > 0) {
            if (unformat_float($data['grade'], true) === false && (!empty($data['grade']))) {
                $errors['grade'] = get_string('invalidfloatforgrade', 'edusign', $data['grade']);
            } else if (unformat_float($data['grade']) > $instance->grade) {
                $errors['grade'] = get_string('gradeabovemaximum', 'edusign', $instance->grade);
            } else if (unformat_float($data['grade']) < 0) {
                $errors['grade'] = get_string('gradebelowzero', 'edusign');
            }
        } else {
            // This is a scale.
