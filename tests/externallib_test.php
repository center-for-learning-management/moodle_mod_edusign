...
require_once($CFG->dirroot . '/mod/edusign/externallib.php');
require_once(__DIR__ . '/fixtures/testable_edusign.php');

/**
 * External mod edusign functions unit tests
 *
 * @package mod_edusign
 * @category external
 ...
 */
class mod_edusign_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_get_grades() {
        ...
        // Create a course and edusignment.
        ...
        $edusigndata['course'] = $course->id;
        $edusigndata['name'] = 'lightwork edusignment';

        $edusign = self::getDataGenerator()->create_module('edusign', $edusigndata);
        ...
        $roleid = $this->edusignUserCapability('moodle/course:viewparticipants', $context->id, 3);
        $context = context_module::instance($edusign->cmid);
        $this->edusignUserCapability('mod/edusign:viewgrades', $context->id, $roleid);
        ...
        $DB->insert_record('edusign_submission', $submission);

        $grade = new stdClass();
        $grade->edusignment = $edusign->id;
        ...
        $DB->insert_record('edusign_grades', $grade);
        ...
        $edusignmentids[] = $edusign->id;
        $result = mod_edusign_external::get_grades($edusignmentids);

        // We need to execute the return values cleaning process to simulate the web service server.
        $result = external_api::clean_returnvalue(mod_edusign_external::get_grades_returns(), $result);

        // Check that the correct grade information for the student is returned.
        $this->assertEquals(1, count($result['edusignments']));
        $edusignment = $result['edusignments'][0];
        $this->assertEquals($edusign->id, $edusignment['edusignmentid']);
        // Should only get the last grade for this student.
        $this->assertEquals(1, count($edusignment['grades']));
        $grade = $edusignment['grades'][0];
    ...
    /**
     * Test get_edusignments
     */
    public function test_get_edusignments() {
        global $DB, $USER, $CFG;
        ...
        // Create the edusignment module with links to a filerecord.
        $edusign1 = self::getDataGenerator()->create_module('edusign', array(
            'course' => $course1->id,
            'name' => 'lightwork edusignment',
            'intro' => 'the edusignment intro text here <a href="@@PLUGINFILE@@/intro.txt">link</a>',
        ...
        $context = context_module::instance($edusign1->cmid);
        $filerecord = array('component' => 'mod_edusign', 'filearea' => 'intro', 'contextid' => $context->id, 'itemid' => 0,
        ...
        $roleid = $this->edusignUserCapability('moodle/course:view', $context->id);
        $context = context_module::instance($edusign1->cmid);
        $this->edusignUserCapability('mod/edusign:view', $context->id, $roleid);
        ...
        $result = mod_edusign_external::get_edusignments();

        // We need to execute the return values cleaning process to simulate the web service server.
        $result = external_api::clean_returnvalue(mod_edusign_external::get_edusignments_returns(), $result);

        // Check the course and edusignment are returned.
        ...
        $this->assertEquals(1, count($course['edusignments']));
        $edusignment = $course['edusignments'][0];
        $this->assertEquals($edusign1->id, $edusignment['id']);
        $this->assertEquals($course1->id, $edusignment['course']);
        $this->assertEquals('lightwork edusignment', $edusignment['name']);
        $this->assertContains('the edusignment intro text here', $edusignment['intro']);
        $this->assertNotEmpty($edusignment['configs']);
        // Check the url of the file attatched.
        $this->assertRegExp('@"' . $CFG->wwwroot . '/webservice/pluginfile.php/\d+/mod_edusign/intro/intro\.txt"@', $edusignment['intro']);
        $this->assertEquals(1, $edusignment['markingworkflow']);
        $this->assertEquals(1, $edusignment['markingallocation']);
        $this->assertEquals(0, $edusignment['preventsubmissionnotingroup']);

        $this->assertCount(1, $edusignment['introattachments']);
        $this->assertEquals('introattachment.txt', $edusignment['introattachments'][0]['filename']);

        // Now, hide the descritption until the submission from date.
        $DB->set_field('edusign', 'alwaysshowdescription', 0, array('id' => $edusign1->id));
        $DB->set_field('edusign', 'allowsubmissionsfromdate', time() + DAYSECS, array('id' => $edusign1->id));

        $result = mod_edusign_external::get_edusignments(array($course1->id));
        ...
        $this->assertEquals($course1->id, $edusignment['course']);
        $this->assertEquals('lightwork edusignment', $edusignment['name']);
        $this->assertArrayNotHasKey('intro', $edusignment);
        $this->assertArrayNotHasKey('introattachments', $edusignment);
        $this->assertEquals(1, $edusignment['markingworkflow']);
        $this->assertEquals(1, $edusignment['markingallocation']);
        $this->assertEquals(0, $edusignment['preventsubmissionnotingroup']);

        $result = mod_edusign_external::get_edusignments(array($course2->id));
        ...
        $result = mod_edusign_external::get_edusignments(array($course1->id), array(), true);
        $result = external_api::clean_returnvalue(mod_edusign_external::get_edusignments_returns(), $result);
    ...
    /**
     * Test get_edusignments with submissionstatement.
     */
    public function test_get_edusignments_with_submissionstatement() {
        global $DB, $USER, $CFG;

        $this->resetAfterTest(true);

        // Setup test data. Create 2 edusigns, one with requiresubmissionstatement and the other without it.
        $course = $this->getDataGenerator()->create_course();
        $edusign = $this->getDataGenerator()->create_module('edusign', array(
            'course' => $course->id,
            'requiresubmissionstatement' => 1
        ));
        $edusign2 = $this->getDataGenerator()->create_module('edusign', array('course' => $course->id));
        ...
        set_config('submissionstatement', $submissionstatement, 'edusign');
        ...
        // Check that the amount of courses and edusignments is right.
        $this->assertCount(1, $result['courses']);
        $edusignmentsret = $result['courses'][0]['edusignments'];
        $this->assertCount(2, $edusignmentsret);

        // Order the returned edusignments by ID.
        usort($edusignmentsret, function($a, $b) {
            return strcmp($a['id'], $b['id']);
        });

        // Check that the first edusign contains the submission statement.
        $edusignmentret = $edusignmentsret[0];
        $this->assertEquals($edusign->id, $edusignmentret['id']);
        $this->assertEquals(1, $edusignmentret['requiresubmissionstatement']);
        $this->assertEquals($submissionstatement, $edusignmentret['submissionstatement']);

        // Check that the second edusign does NOT contain the submission statement.
        $edusignmentret = $edusignmentsret[1];
        $this->assertEquals($edusign2->id, $edusignmentret['id']);
        $this->assertEquals(0, $edusignmentret['requiresubmissionstatement']);
        $this->assertArrayNotHasKey('submissionstatement', $edusignmentret);
    }
    ...
    public function test_get_submissions() {
        ...
        $edusigndata['course'] = $course1->id;
        $edusigndata['name'] = 'lightwork edusignment';

        $edusign1 = self::getDataGenerator()->create_module('edusign', $edusigndata);
        ...
        $submission = new stdClass();
        $submission->edusignment = $edusign1->id;
        $submission->userid = $student->id;
        $submission->timecreated = time();
        ...
        $onlinetextsubmission->edusignment = $edusign1->id;
        $DB->insert_record('edusignsubmission_onlinetext', $onlinetextsubmission);
        ...
        $edusignmentids[] = $edusign1->id;
        $result = mod_edusign_external::get_submissions($edusignmentids);
        $result = external_api::clean_returnvalue(mod_edusign_external::get_submissions_returns(), $result);

        // Check the online text submission is NOT returned because the student is not yet enrolled in the course.
        $this->assertEquals(1, count($result['edusignments']));
        $edusignment = $result['edusignments'][0];
        $this->assertEquals($edusign1->id, $edusignment['edusignmentid']);
        $this->assertEquals(0, count($edusignment['submissions']));
        ...
        $this->assertEquals(1, count($edusignment['submissions']));
        $submission = $edusignment['submissions'][0];
    ...
    public function test_get_user_flags() {
        ...
        $roleid = $this->edusignUserCapability('moodle/course:viewparticipants', $context->id, 3);
        $context = context_module::instance($edusign->cmid);
        $this->edusignUserCapability('mod/edusign:grade', $context->id, $roleid);
        ...
        $userflag->edusignment = $edusign->id;
        ...
        $DB->insert_record('edusign_user_flags', $userflag);

        $edusignmentids[] = $edusign->id;
        $result = mod_edusign_external::get_user_flags($edusignmentids);

        // We need to execute the return values cleaning process to simulate the web service server.
        $result = external_api::clean_returnvalue(mod_edusign_external::get_user_flags_returns(), $result);
        ...
        $this->assertEquals(1, count($edusignment['userflags']));
        $userflag = $edusignment['userflags'][0];
    ...
    public function test_get_user_mappings() {
        ...
        $roleid = $this->edusignUserCapability('moodle/course:viewparticipants', $context->id, 3);
        $context = context_module::instance($edusign->cmid);
        $this->edusignUserCapability('mod/edusign:revealidentities', $context->id, $roleid);
        ...
        $mapping->edusignment = $edusign->id;
        $mapping->userid = $student->id;

        $DB->insert_record('edusign_user_mapping', $mapping);

        $edusignmentids[] = $edusign->id;
        $result = mod_edusign_external::get_user_mappings($edusignmentids);

        // We need to execute the return values cleaning process to simulate the web service server.
        $result = external_api::clean_returnvalue(mod_edusign_external::get_user_mappings_returns(), $result);
        ...
        $this->assertEquals(1, count($edusignment['mappings']));
        $mapping = $edusignment['mappings'][0];
    ...
    public function test_lock_submissions() {
        ...
        $this->resetAfterTest(true);
        // Create a course and edusignment and users.
        ...
        $result = mod_edusign_external::lock_submissions($instance->id, $students);
        $result = external_api::clean_returnvalue(mod_edusign_external::lock_submissions_returns(), $result);
    ...
    public function test_unlock_submissions() {
        ...
        $result = mod_edusign_external::unlock_submissions($instance->id, $students);
        $result = external_api::clean_returnvalue(mod_edusign_external::unlock_submissions_returns(), $result);
    ...
    public function test_submit_for_grading() {
        ...
        $result = mod_edusign_external::submit_for_grading($instance->id, false);
        $result = external_api::clean_returnvalue(mod_edusign_external::submit_for_grading_returns(), $result);

        // Should be 1 fail because the submission statement was not aceptted.
        $this->assertEquals(1, count($result));

        $result = mod_edusign_external::submit_for_grading($instance->id, true);
        $result = external_api::clean_returnvalue(mod_edusign_external::submit_for_grading_returns(), $result);

        // Check for 0 warnings.
        $this->assertEquals(0, count($result));

        $submission = $edusign->get_user_submission($student1->id, false);

        $this->assertEquals(edusign_SUBMISSION_STATUS_SUBMITTED, $submission->status);
    }
    ...
    public function test_save_user_extensions() {
        ...
        $this->setUser($teacher);
        $result = mod_edusign_external::save_user_extensions($instance->id, array($student1->id), array($now, $tomorrow));
        $result = external_api::clean_returnvalue(mod_edusign_external::save_user_extensions_returns(), $result);
        $this->assertEquals(1, count($result));

        $this->setUser($teacher);
        $result = mod_edusign_external::save_user_extensions($instance->id, array($student1->id), array($yesterday - 10));
        $result = external_api::clean_returnvalue(mod_edusign_external::save_user_extensions_returns(), $result);
        $this->assertEquals(1, count($result));

        $this->setUser($teacher);
        $result = mod_edusign_external::save_user_extensions($instance->id, array($student1->id), array($tomorrow));
        $result = external_api::clean_returnvalue(mod_edusign_external::save_user_extensions_returns(), $result);
        $this->assertEquals(0, count($result));
    ...
    public function test_reveal_identities() {
        ...
        $this->setUser($student1);
        $result = mod_edusign_external::reveal_identities($instance->id);
        $result = external_api::clean_returnvalue(mod_edusign_external::reveal_identities_returns(), $result);
        $this->assertEquals(1, count($result));
    ...
    public function test_revert_submissions_to_draft() {
        ...
        $this->setUser($teacher);
        $students = array($student1->id, $student2->id);
        $result = mod_edusign_external::revert_submissions_to_draft($instance->id, array($student1->id));
        $result = external_api::clean_returnvalue(mod_edusign_external::revert_submissions_to_draft_returns(), $result);
    ...
    public function test_save_submission() {
        ...
        $this->assertEquals(get_string('duedatereached', 'edusign'), $result[0]['item']);
    }
    ...
    public function test_save_grade() {
        ...
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_edusign');
        $params['course'] = $course->id;
        $params['edusignfeedback_file_enabled'] = 1;
        $params['edusignfeedback_comments_enabled'] = 1;
        $instance = $generator->create_instance($params);
        $cm = get_coursemodule_from_instance('edusign', $instance->id);
        $context = context_module::instance($cm->id);
        ...
        $result = mod_edusign_external::get_grades(array($instance->id));
        $result = external_api::clean_returnvalue(mod_edusign_external::get_grades_returns(), $result);

        $this->assertEquals($result['edusignments'][0]['grades'][0]['grade'], '50.0');
    }
    ...
    public function test_save_grades_with_advanced_grading() {
        ...
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_edusign');
        $params['course'] = $course->id;
        $params['edusignfeedback_file_enabled'] = 0;
        $params['edusignfeedback_comments_enabled'] = 0;
        $instance = $generator->create_instance($params);
        $cm = get_coursemodule_from_instance('edusign', $instance->id);
        $context = context_module::instance($cm->id);
        ...
        $result = mod_edusign_external::save_grades($instance->id, false, $grades);
        $this->assertNull($result);

        $student1grade = $DB->get_record('edusign_grades',
                                         array('userid' => $student1->id, 'edusignment' => $instance->id),
                                         '*',
                                         MUST_EXIST);
        $this->assertEquals($student1grade->grade, '50.0');

        $student2grade = $DB->get_record('edusign_grades',
                                         array('userid' => $student2->id, 'edusignment' => $instance->id),
                                         '*',
                                         MUST_EXIST);
    ...
    public function test_save_grades_with_group_submission() {
        ...
        $groupingdata['name'] = 'Group edusignment grouping';
        ...
        $result = mod_edusign_external::save_grades($instance->id, true, $grades1);
        $result = external_api::clean_returnvalue(mod_edusign_external::save_grades_returns(), $result);
        ...
        $result = mod_edusign_external::save_grades($instance->id, true, $grades2);
        $result = external_api::clean_returnvalue(mod_edusign_external::save_grades_returns(), $result);
        // There should be no warnings.
        $this->assertEquals(0, count($result));

        $student3grade = $DB->get_record('edusign_grades',
                                         array('userid' => $student3->id, 'edusignment' => $instance->id),
                                         '*',
                                         MUST_EXIST);
        $this->assertEquals($student3grade->grade, '50.0');

        $student4grade = $DB->get_record('edusign_grades',
                                         array('userid' => $student4->id, 'edusignment' => $instance->id),
                                         '*',
                                         MUST_EXIST);
    ...
    public function test_copy_previous_attempt() {
        ...
        $params['course'] = $course->id;
        $params['edusignsubmission_onlinetext_enabled'] = 1;
        $params['edusignsubmission_file_enabled'] = 0;
        $params['edusignfeedback_file_enabled'] = 0;
        ...
        $result = mod_edusign_external::copy_previous_attempt($instance->id);
        $result = external_api::clean_returnvalue(mod_edusign_external::copy_previous_attempt_returns(), $result);
        // No warnings.
        $this->assertEquals(0, count($result));

        $this->setUser($teacher);
        $result = mod_edusign_external::get_submissions(array($instance->id));
        $result = external_api::clean_returnvalue(mod_edusign_external::get_submissions_returns(), $result);

        // Check we are now on the second attempt.
        $this->assertEquals($result['edusignments'][0]['submissions'][0]['attemptnumber'], 1);
        // Check the plugins data is not empty.
        $this->assertNotEmpty($result['edusignments'][0]['submissions'][0]['plugins']);
    ...
    public function test_set_user_flags() {
        ...
        $createduserflags = mod_edusign_external::set_user_flags($edusign->id, $userflags);
        // We need to execute the return values cleaning process to simulate the web service server.
        $createduserflags = external_api::clean_returnvalue(mod_edusign_external::set_user_flags_returns(), $createduserflags);

        $this->assertEquals($student->id, $createduserflags[0]['userid']);
        $createduserflag = $DB->get_record('edusign_user_flags', array('id' => $createduserflags[0]['id']));

        // Confirm that all data was inserted correctly.
        $this->assertEquals($student->id,  $createduserflag->userid);
        $this->assertEquals($edusign->id, $createduserflag->edusignment);
        ...
        $updateduserflags = mod_edusign_external::set_user_flags($edusign->id, $userflags);
        // We need to execute the return values cleaning process to simulate the web service server.
        $updateduserflags = external_api::clean_returnvalue(mod_edusign_external::set_user_flags_returns(), $updateduserflags);

        $this->assertEquals($student->id, $updateduserflags[0]['userid']);
        $updateduserflag = $DB->get_record('edusign_user_flags', array('id' => $updateduserflags[0]['id']));

        // Confirm that all data was updated correctly.
        $this->assertEquals($student->id,  $updateduserflag->userid);
        $this->assertEquals($edusign->id, $updateduserflag->edusignment);
    ...
    public function test_view_grading_table_invalid_instance() {
        ...
        // Setup test data.
        $course = $this->getDataGenerator()->create_course();
        $edusign = $this->getDataGenerator()->create_module('edusign', array('course' => $course->id));
        $context = context_module::instance($edusign->cmid);
        $cm = get_coursemodule_from_instance('edusign', $edusign->id);

        // Test invalid instance id.
        mod_edusign_external::view_grading_table(0);
    }
    ...
    public function test_view_grading_table_not_enrolled() {
        ...
        mod_edusign_external::view_grading_table($edusign->id);
    }
    ...
    public function test_view_grading_table_correct() {
        ...
        $result = mod_edusign_external::view_grading_table($edusign->id);
        $result = external_api::clean_returnvalue(mod_edusign_external::view_grading_table_returns(), $result);
        ...
        $moodleurl = new \moodle_url('/mod/edusign/view.php', array('id' => $cm->id));
    ...
    public function test_view_grading_table_without_capability() {
        ...
        $teacherrole = $DB->get_record('role', array('shortname' => 'teacher'));
        edusign_capability('mod/edusign:view', CAP_PROHIBIT, $teacherrole->id, $context->id);
    ...
    public function test_subplugins_availability() {
        ...
        // Hide edusignment file submissiong plugin.
        $pluginmanager = new edusign_plugin_manager('edusignsubmission');
        $pluginmanager->hide_plugin('file');
        $parameters = mod_edusign_external::save_submission_parameters();
        ...
        $pluginmanager = new edusign_plugin_manager('edusignfeedback');
        $pluginmanager->hide_plugin('file');

        $parameters = mod_edusign_external::save_grade_parameters();
        ...
        $this->assertTrue(isset($parameters->keys['plugindata']->keys['edusignfeedbackcomments_editor']));
        $this->assertEquals(VALUE_OPTIONAL, $parameters->keys['plugindata']->keys['edusignfeedbackcomments_editor']->required);
    }
    ...
    public function test_view_submission_status() {
        ...
        // Test invalid instance id.
        try {
            mod_edusign_external::view_submission_status(0);
            $this->fail('Exception expected due to invalid mod_edusign instance id.');
        ...
        $this->setUser($user);
        try {
            mod_edusign_external::view_submission_status($edusign->id);
        ...
        $result = mod_edusign_external::view_submission_status($edusign->id);
        $result = external_api::clean_returnvalue(mod_edusign_external::view_submission_status_returns(), $result);
        ...
        // Test user with no capabilities.
        // We need a explicit prohibit since this capability is only defined in authenticated user and guest roles.
        edusign_capability('mod/edusign:view', CAP_PROHIBIT, $studentrole->id, $context->id);
        accesslib_clear_all_caches_for_unit_testing();
        course_modinfo::clear_instance_cache();
    ...
    private function create_submission_for_testing_status($submitforgrading = false) {
        ...
        $params = array(
            'course' => $course->id,
            'edusignsubmission_file_maxfiles' => 1,
            'edusignsubmission_file_maxsizebytes' => 1024 * 1024,
            'edusignsubmission_onlinetext_enabled' => 1,
            'edusignsubmission_file_enabled' => 1,
            'submissiondrafts' => 1,
            'edusignfeedback_file_enabled' => 1,
        ...
        if ($submitforgrading) {
            ...
            $edusign->submit_for_grading($data, $notices);
        }

        return array($edusign, $instance, $student1, $student2, $teacher, $group1, $group2);
    }
    ...
    public function test_get_submission_status_in_draft_status() {
        ...
        list($edusign, $instance, $student1, $student2, $teacher, $g1, $g2) = $this->create_submission_for_testing_status();
        $studentsubmission = $edusign->get_user_submission($student1->id, true);

        $result = mod_edusign_external::get_submission_status($edusign->get_instance()->id);
        // We expect debugging because of the $PAGE object, this won't happen in a normal WS request.
        $this->assertDebuggingCalled();

        $result = external_api::clean_returnvalue(mod_edusign_external::get_submission_status_returns(), $result);
        ...
        $this->assertEquals($edusign->get_instance()->id, $result['lastattempt']['submission']['edusignment']);
        ...
        list($expectedtext, $expectedformat) = external_format_text($onlinetext, FORMAT_HTML, $edusign->get_context()->id,
                'edusignsubmission_onlinetext', edusignSUBMISSION_ONLINETEXT_FILEAREA, $studentsubmission->id);
    ...
    public function test_get_submission_status_in_submission_status() {
        ...
        list($edusign, $instance, $student1, $student2, $teacher, $g1, $g2) = $this->create_submission_for_testing_status(true);
    ...
    public function test_get_submission_status_in_submission_status_for_teacher() {
        ...
        $this->setUser($teacher);
        // First one group.
        $result = mod_edusign_external::get_submission_status($edusign->get_instance()->id, 0, $g1->id);
        // We expect debugging because of the $PAGE object, this won't happen in a normal WS request.
        $this->assertDebuggingCalled();
        $result = external_api::clean_returnvalue(mod_edusign_external::get_submission_status_returns(), $result);
        ...
        // Second group.
        $result = mod_edusign_external::get_submission_status($edusign->get_instance()->id, 0, $g2->id);
        $result = external_api::clean_returnvalue(mod_edusign_external::get_submission_status_returns(), $result);
        $this->assertCount(0, $result['warnings']);
        $this->assertEquals(1, $result['gradingsummary']['participantcount']);
        ...
        list($edusign, $instance, $student1, $student2, $teacher, $g1, $g2) = $this->create_submission_for_testing_status(false);
        $this->setUser($teacher);
        $result = mod_edusign_external::get_submission_status($edusign->get_instance()->id, 0, $g1->id);
        $result = external_api::clean_returnvalue(mod_edusign_external::get_submission_status_returns(), $result);
        $this->assertCount(0, $result['warnings']);
    ...
    public function test_get_submission_status_in_reopened_status() {
        ...
        $edusign->testable_process_add_attempt($student1->id);
    ...
    public function test_get_submission_status_access_control() {
        ...
        // Access control test.
        mod_edusign_external::get_submission_status($edusign->get_instance()->id, $student1->id);

    }

    /**
     * get_participant should throw an excaption if the requested edusignment doesn't exist.
     *
     * @expectedException moodle_exception
     */
    public function test_get_participant_no_edusignment() {
        $this->resetAfterTest(true);
        mod_edusign_external::get_participant('-1', '-1', false);
    }

    /**
     * get_participant should throw a require_login_exception if the user doesn't have access
     * to view edusignments.
     *
     * @expectedException require_login_exception
     */
    public function test_get_participant_no_view_capability() {
        ...
        $result = $this->create_edusign_with_student_and_teacher();
        $edusign = $result['edusign'];
        ...
        $this->setUser($student);
        edusign_capability('mod/edusign:view', CAP_PROHIBIT, $studentrole->id, $context->id, true);

        mod_edusign_external::get_participant($edusign->id, $student->id, false);
    }

    /**
     * get_participant should throw a required_capability_exception if the user doesn't have access
     * to view edusignment grades.
     *
     * @expectedException required_capability_exception
     */
    public function test_get_participant_no_grade_capability() {
        ...
        $this->setUser($teacher);
        edusign_capability('mod/edusign:viewgrades', CAP_PROHIBIT, $teacherrole->id, $context->id, true);
        edusign_capability('mod/edusign:grade', CAP_PROHIBIT, $teacherrole->id, $context->id, true);
        accesslib_clear_all_caches_for_unit_testing();
    ...
    public function test_get_participant_no_participant() {
        ...
        $result = $this->create_edusign_with_student_and_teacher(array('blindmarking' => true));
        ...
        $result = mod_edusign_external::get_participant($edusign->id, $student->id, false);
        $result = external_api::clean_returnvalue(mod_edusign_external::get_participant_returns(), $result);
    }

    /**
     * get_participant should return a summarised list of details with a different fullname if blind
     * marking is on for the requested edusignment.
     */
    public function test_get_participant_blind_marking() {
        ...
        $result = mod_edusign_external::get_participant($edusign->id, $student->id, true);
        $result = external_api::clean_returnvalue(mod_edusign_external::get_participant_returns(), $result);
    ...
    public function test_get_participant_no_user() {
        ...
        // Create an edusign instance to save a submission.
        set_config('submissionreceipts', 0, 'edusign');

        $cm = get_coursemodule_from_instance('edusign', $edusignmodule->id);
        $context = context_module::instance($cm->id);
        ...
        $result = mod_edusign_external::get_participant($edusignmodule->id, $student->id, false);
        $result = external_api::clean_returnvalue(mod_edusign_external::get_participant_returns(), $result);
        $this->assertEquals($student->id, $result['id']);
    ...
    public function test_get_participant_group_submission() {
        ...
        $result = $this->create_edusign_with_student_and_teacher(array(
            'edusignsubmission_onlinetext_enabled' => 1,
            'teamsubmission' => 1
        ));
        $edusignmodule = $result['edusign'];
        ...
        $this->setUser($student);
        $submission = $edusign->get_group_submission($student->id, $group->id, true);
    ...
    /**
     * Test for mod_edusign_external::list_participants().
     *
    ...
    public function test_list_participants_user_info_with_special_characters() {
        ...
        $data = $this->create_edusign_with_student_and_teacher();
        $edusignment = $data['edusign'];
        ...
        $this->setUser($teacher);
        $participants = mod_edusign_external::list_participants($edusignment->id, 0, '', 0, 0, false, true);
        $participants = external_api::clean_returnvalue(mod_edusign_external::list_participants_returns(), $participants);
        $this->assertCount(1, $participants);

        // Asser that we have a valid response data.
        $response = external_api::clean_returnvalue(mod_edusign_external::list_participants_returns(), $participants);
        ...
        $participants = mod_edusign_external::list_participants($edusignment->id, 0, '', 0, 0, false, false);
        $participants = external_api::clean_returnvalue(mod_edusign_external::list_participants_returns(), $participants);
    ...
    /**
     * Test for the type of the user-related properties in mod_edusign_external::list_participants_returns().
     */
    public function test_list_participants_returns_user_property_types() {
        ...
        // Get returns description for mod_edusign_external::list_participants_returns().
        $listreturns = mod_edusign_external::list_participants_returns();
    ...
    /**
     * Create a a course, edusignment module instance, student and teacher and enrol them in
     * the course.
     *
     * @param array $params parameters to be provided to the edusignment module creation
     * @return array containing the course, edusignment module, student and teacher
     */
    private function create_edusign_with_student_and_teacher($params = array()) {
        global $DB;
        ...
        $params = array_merge(array(
            'course' => $course->id,
            'name' => 'edusignment',
            'intro' => 'edusignment intro text',
        ), $params);

        // Create a course and edusignment and users.
        $edusign = $this->getDataGenerator()->create_module('edusign', $params);
        ...
        edusign_capability('mod/edusign:view', CAP_ALLOW, $teacherrole->id, $context->id, true);
        edusign_capability('mod/edusign:viewgrades', CAP_ALLOW, $teacherrole->id, $context->id, true);
        edusign_capability('mod/edusign:grade', CAP_ALLOW, $teacherrole->id, $context->id, true);
        accesslib_clear_all_caches_for_unit_testing();
    ...
    /**
     * Test test_view_edusign
     */
    public function test_view_edusign() {
        global $CFG;
        ...
        $result = mod_edusign_external::view_edusign($edusign->id);
        $result = external_api::clean_returnvalue(mod_edusign_external::view_edusign_returns(), $result);
