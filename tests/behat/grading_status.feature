@mod @mod_edusign
Feature: View the grading status of an edusignment
  In order to test the grading status for edusignments is displaying correctly
  As a student
  ...
  Scenario: View the grading status for an edusignment with marking workflow enabled
    ...
    # The grade should also remain displayed as it's stored in the edusign DB tables, but the final grade should be empty.
    And "Student 1" row "Grade" column of "generaltable" table should contain "50.00"
    And "Student 1" row "Final grade" column of "generaltable" table should contain "-"
    And I log out

  @javascript
  Scenario: View the grading status for an edusignment with marking workflow disabled
    # Add the edusignment.
    And I log in as "teacher1"
    And I am on "Course 1" course homepage with editing mode on
    And I add a "edusignment" to section "1" and I fill the form with:
