...
class mod_edusign_locallib_participants extends advanced_testcase {
    public function test_list_participants_blind_marking() {
        ...
        edusign::allocate_unique_ids($edusign->get_instance()->id);

        $participants = $edusign->list_participants(null, false);
        ...
        $table = new edusign_grading_table($edusign, 10, '', 0, false);
        $table->setup();
        $table->query_db(10);
        ...
        $newparticipants = $edusign->list_participants(null, false);
        ...
        static::helper_add_submission($edusign, $participants[$keys[0]], $data, 'onlinetext');
        $edusign = new edusign($context, $cm, $course);
        $newparticipants = $edusign->list_participants(null, false);
    ...
    public function helper_add_submission($edusign, $user, $data, $type) {
        ...
        $rcm->invokeArgs($edusign, [$submission, $user->id, true, false]);

        $plugin = $edusign->get_submission_plugin_by_type($type);
