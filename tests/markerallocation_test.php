...
/**
 * Unit tests for (some of) mod/edusign/markerallocaion_test.php.
 *
 * @package    mod_edusign
 * @category   test
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_edusign_markerallocation_testcase extends advanced_testcase {
    ...
    public function test_markerusers() {
        ...
        $edusignelement = $this->getDataGenerator()->create_module('edusign', $record, $modulesettings);

        $coursesectionid = course_add_cm_to_section($this->course->id, $edusignelement->id, 1);
        ...
        if (!empty($managerrole)) {
            // By default the context of the system is edusigned.
            $idedusignment = $this->getDataGenerator()->role_edusign($managerrole->id, $user4->id);
        }
        ...
        $oldmarkers = get_users_by_capability(context_course::instance($this->course->id), 'mod/edusign:grade', '', $sort);
        // New code, it must return 2 users: teacher1 and teacher2.
        $newmarkers = get_enrolled_users(context_course::instance($this->course->id), 'mod/edusign:grade', 0, 'u.*', $sort);
