...
/**
 * Base class for unit tests for mod_edusign.
 *
 * @package    mod_edusign
...
use \mod_edusign\privacy\provider;

/**
 * Unit tests for mod/edusign/classes/privacy/
 *
 * @copyright  2018 Adrian Greeve <adrian@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_edusign_privacy_testcase extends provider_testcase
{
    ...
    public function test_get_contexts_for_userid()
    {
        ...
        // Need a second user to create content in other edusignments.
        $user2 = $this->getDataGenerator()->create_user();
        $this->getDataGenerator()->enrol_user($user2->id, $course2->id, 'student');

        // Create multiple edusignments.
        // edusignment with a text submission.
        $edusign1 = $this->create_instance(['course' => $course1]);
        // edusignment two in a different course that the user is not enrolled in.
        $edusign2 = $this->create_instance(['course' => $course2]);
        // edusignment three has an entry in the override table.
        $edusign3 = $this->create_instance(['course' => $course3, 'cutoffdate' => time()]);
        // edusignment four - blind marking.
        $edusign4 = $this->create_instance(['course' => $course1, 'blindmarking' => 1]);
        // edusignment five - user flags.
        $edusign5 = $this->create_instance(['course' => $course3]);

        // Override has to be manually inserted into the DB.
        $overridedata = new \stdClass();
        $overridedata->edusignid = $edusign3->get_instance()->id;
        $overridedata->userid = $user1->id;
        $overridedata->duedate = time();
        $DB->insert_record('edusign_overrides', $overridedata);
        // edusign unique id for blind marking in edusignment four for user 1.
        \edusign::get_uniqueid_for_user_static($edusign4->get_instance()->id, $user1->id);
        // Create an entry in the user flags table.
        $edusign5->get_user_flags($user1->id, true);
        ...
        $submission->edusignment = $edusign1->get_instance()->id;
        ...
        $this->setUser($user1);
        $notices = [];
        $edusign1->save_submission($submission, $notices);

        // Create a submission for the second edusignment.
        $submission->edusignment = $edusign2->get_instance()->id;
        $submission->userid = $user2->id;
        $this->setUser($user2);
        $edusign2->save_submission($submission, $notices);
    ...
    public function test_get_users_in_context()
    {
        ...
        $overridedata = new \stdClass();
        $overridedata->edusignid = $edusign1->get_instance()->id;
        ...
        $submission = $this->create_submission($edusign1, $user3, $submissiontext);

        $submissiontext = 'My first submission';
        $submission = $this->create_submission($edusign1, $user4, $submissiontext);
        ...
        $data->grade = $grade;
        $data->edusignfeedbackcomments_editor = ['text' => $teachercommenttext, 'format' => FORMAT_MOODLE];

        // Give the submission a grade.
        $edusign1->save_grade($user4->id, $data);

        $userlist = new \core_privacy\local\request\userlist($context, 'edusign');
    ...
    public function test_export_user_data_student()
    {
        ...
        $submissiontext = 'My first submission';
        $submission = $this->create_submission($edusign, $user, $submissiontext);

        $this->setUser($teacher);

        $overridedata = new \stdClass();
        $overridedata->edusignid = $edusign->get_instance()->id;
        ...
        // Give the submission a grade.
        $edusign->save_grade($user->id, $data);

        $submissiontext2 = 'My second submission';
        $submission = $this->create_submission($edusign, $user, $submissiontext2, 1);
        ...
        $data->attemptnumber = 1;
        $data->grade = $grade2;
        $data->edusignfeedbackcomments_editor = ['text' => $teachercommenttext2, 'format' => FORMAT_MOODLE];
        ...
        $approvedlist = new approved_contextlist($user, 'mod_edusign', [$context->id, $coursecontext->id]);
        provider::export_user_data($approvedlist);

        // Check that we have general details about the edusignment.
        $this->assertEquals('edusign 1', $writer->get_data()->name);
        // Check Submissions.
    ...
    public function test_export_user_data_teacher()
    {
        ...
        $submissiontext = 'My first submission';
        $submission = $this->create_submission($edusign, $user1, $submissiontext);
        ...
        // Give the submission a grade.
        $edusign->save_grade($user1->id, $data);

        // Create and grade some submissions from the students.
        $submissiontext2 = 'My first submission for user 2';
        $submission = $this->create_submission($edusign, $user2, $submissiontext2);
        ...
        // Give the submission a grade.
        $edusign->save_grade($user2->id, $data);

        // Create and grade some submissions from the students.
        $submissiontext3 = 'My second submission for user 2';
        $submission = $this->create_submission($edusign, $user2, $submissiontext3, 1);
        ...
        $data->attemptnumber = 1;
        $data->grade = $grade3;
        $data->edusignfeedbackcomments_editor = ['text' => $teachercommenttext3, 'format' => FORMAT_MOODLE];
        ...
        // Set up some flags.
        $duedate = time();
        $flagdata = $edusign->get_user_flags($teacher->id, true);
        $flagdata->mailed = 1;
        $flagdata->extensionduedate = $duedate;
        $edusign->update_user_flags($flagdata);

        $writer = writer::with_context($context);
        $this->assertFalse($writer->has_any_data());

        // The student should have some text submitted.
        $approvedlist = new approved_contextlist($teacher, 'mod_edusign', [$context->id, $coursecontext->id]);
        provider::export_user_data($approvedlist);
    ...
    public function test_delete_data_for_all_users_in_context()
    {
        ...
        // Delete all user data for this edusignment.
        provider::delete_data_for_all_users_in_context($context);

        // Check all relevant tables.
        $records = $DB->get_records('edusign_submission');
        $this->assertEmpty($records);
        $records = $DB->get_records('edusign_grades');
        $this->assertEmpty($records);
        $records = $DB->get_records('edusignsubmission_onlinetext');
        $this->assertEmpty($records);
        $records = $DB->get_records('edusignfeedback_comments');
        $this->assertEmpty($records);

        // Check that overrides and the calendar events are deleted.
        $records = $DB->get_records('event');
        $this->assertEmpty($records);
        $records = $DB->get_records('edusign_overrides');
        $this->assertEmpty($records);
    }
    ...
    public function test_delete_data_for_user()
    {
        ...
        // Create and grade some submissions from the students.
        $submissiontext = 'My first submission';
        $submission1 = $this->create_submission($edusign, $user1, $submissiontext);
        ...
        // Create and grade some submissions from the students.
        $submissiontext2 = 'My first submission for user 2';
        $submission2 = $this->create_submission($edusign, $user2, $submissiontext2);
        ...
        // Create and grade some submissions from the students.
        $submissiontext3 = 'My second submission for user 2';
        $submission3 = $this->create_submission($edusign, $user2, $submissiontext3, 1);
        ...
        $approvedlist = new approved_contextlist($user2, 'mod_edusign', [$context->id, $coursecontext->id]);
        ...
        // Check calendar events as well as edusign overrides.
        $records = $DB->get_records('event');
        $this->assertCount(1, $records);
        $record = array_shift($records);
        // The remaining event should be for user 1.
        $this->assertEquals($user1->id, $record->userid);
        // Now for edusign_overrides
        $records = $DB->get_records('edusign_overrides');
        $this->assertCount(1, $records);
        $record = array_shift($records);
        // The remaining event should be for user 1.
    ...
    public function test_delete_data_for_users()
    {
        ...
        // Grading and providing feedback as a teacher.
        $user5 = $this->getDataGenerator()->create_user();
        // This user has entries in edusignment 2 and should not have their data deleted.
        $user6 = $this->getDataGenerator()->create_user();
        ...
        $submissiontext = 'My first submission';
        $submission = $this->create_submission($edusign2, $user6, $submissiontext);
        ...
        // Give the submission a grade.
        $edusign2->save_grade($user6->id, $data);

        // Check data is in place.
        $data = $DB->get_records('edusign_submission');
        ...
        $data = $DB->get_records('edusign_grades');
        // Two entries in edusign_grades, one for each grade given.
        $this->assertCount(2, $data);

        $data = $DB->get_records('edusign_overrides');
        $this->assertCount(1, $data);

        $data = $DB->get_records('comments');
        $this->assertCount(1, $data);

        $userlist = new \core_privacy\local\request\approved_userlist($context, 'edusign', [$user1->id, $user2->id]);
        ...
        $userlist = new \core_privacy\local\request\approved_userlist($context, 'edusign', [$user3->id, $user5->id]);
        provider::delete_data_for_users($userlist);
