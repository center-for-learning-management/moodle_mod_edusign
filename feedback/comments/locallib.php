 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class edusign_feedback_comments extends edusign_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'edusignfeedback_comments');
    }
    ...
    public function is_feedback_modified(stdClass $grade, stdClass $data) {
        ...
        if ($commenttext == $data->edusignfeedbackcomments_editor['text']) {
            return false;
        } else {
            return true;
        }
    ...
    public function get_editor_fields() {
        return array('comments' => get_string('pluginname', 'edusignfeedback_comments'));
    }
    ...
    public function set_editor_text($name, $value, $gradeid) {
        ...
        if ($name == 'comments') {
            ...
            if ($feedbackcomment) {
                ...
                return $DB->update_record('edusignfeedback_comments', $feedbackcomment);
            } else {
                $feedbackcomment = new stdClass();
                $feedbackcomment->commenttext = $value;
                $feedbackcomment->commentformat = FORMAT_HTML;
                $feedbackcomment->grade = $gradeid;
                $feedbackcomment->edusignment = $this->edusignment->get_instance()->id;
                return $DB->insert_record('edusignfeedback_comments', $feedbackcomment) > 0;
            }
        }
    ...
    public function save_settings(stdClass $data) {
        $this->set_config('commentinline', !empty($data->edusignfeedback_comments_commentinline));
        return true;
    }
    ...
    protected function convert_submission_text_to_feedback($submission, $data) {
        ...
        $data->edusignfeedbackcomments_editor['text'] = $text;
        $data->edusignfeedbackcomments_editor['format'] = $format;
    ...
    public function get_form_elements_for_user($grade, MoodleQuickForm $mform, stdClass $data, $userid) {
        ...
        $submission = $this->edusignment->get_user_submission($userid, false);
        ...
        if ($feedbackcomments && !empty($feedbackcomments->commenttext)) {
            $data->edusignfeedbackcomments_editor['text'] = $feedbackcomments->commenttext;
            $data->edusignfeedbackcomments_editor['format'] = $feedbackcomments->commentformat;
        } else {
        ...
        $mform->addElement('editor', 'edusignfeedbackcomments_editor', $this->get_name(), null, null);
    ...
    public function save(stdClass $grade, stdClass $data) {
        ...
        $feedbackcomment = $this->get_feedback_comments($grade->id);
        if ($feedbackcomment) {
            $feedbackcomment->commenttext = $data->edusignfeedbackcomments_editor['text'];
            $feedbackcomment->commentformat = $data->edusignfeedbackcomments_editor['format'];
            return $DB->update_record('edusignfeedback_comments', $feedbackcomment);
        } else {
            $feedbackcomment = new stdClass();
            $feedbackcomment->commenttext = $data->edusignfeedbackcomments_editor['text'];
    ...
    /**
     * Return true if this plugin can upgrade an old Moodle 2.2 edusignment of this type
     * and version.
     *
     ...
     *
     * @param context $oldcontext - the context for the old edusignment
     * @param stdClass $oldedusignment - the data for the old edusignment
    ...
    public function upgrade_settings(context $oldcontext, stdClass $oldedusignment, & $log) {
        if ($oldedusignment->edusignmenttype == 'online') {
            $this->set_config('commentinline', $oldedusignment->var1);
            return true;
        }
        return true;
    }
    ...
    /**
     * Upgrade the feedback from the old edusignment to the new one
     *
    ...
    public function upgrade(context $oldcontext,
        ...
        $feedbackcomments->edusignment = $this->edusignment->get_instance()->id;
        if (!$DB->insert_record('edusignfeedback_comments', $feedbackcomments) > 0) {
            $log .= get_string('couldnotconvertgrade', 'mod_edusign', $grade->userid);
            return false;
        }
     ...
     * of the comment
     *
     * Only one feedback plugin can push comments to the gradebook and that is chosen by the edusignment
     * settings page.
     *
     * @param stdClass $grade The grade
     * @return int
