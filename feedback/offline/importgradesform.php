 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class edusignfeedback_offline_import_grades_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        $renderer = $PAGE->get_renderer('edusign');

        // Visible elements.
        $edusignment = $params['edusignment'];
        ...
        $mform->addElement('header', 'importgrades', get_string('importgrades', 'edusignfeedback_offline'));
        ...
        while ($record = $gradeimporter->next()) {
            ...
            if ($edusignment->is_blind_marking()) {
                $userdesc = get_string('hiddenuser', 'edusign') . $edusignment->get_uniqueid_for_user($user->id);
            }

            $usergrade = $edusignment->get_user_grade($user->id, false);
            ...
            if ($usergrade && $usergrade->grade == $grade) {
                ...
                $skip = true;
            } else if ($edusignment->grading_disabled($user->id)) {
                // Skip grade is locked.
                $skip = true;
            } else if (($edusignment->get_instance()->grade > -1) &&
                    (($grade < 0) || ($grade > $edusignment->get_instance()->grade))) {
                // Out of range.
                $skip = true;
            }
            ...
            if (!$skip) {
                ...
                if (!empty($scaleoptions)) {
                    ...
                    $gradeitem = $edusignment->get_grade_item();
        ...
        if ($update) {
            ...
            $mform->addElement('html', get_string('nochanges', 'edusignfeedback_offline'));
        }

        $mform->addElement('hidden', 'id', $edusignment->get_course_module()->id);
        ...
        $mform->addElement('hidden', 'pluginsubtype', 'edusignfeedback');
