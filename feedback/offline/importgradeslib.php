 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class edusignfeedback_offline_grade_importer {
    ...
    /** @var edusignment $edusignment - the edusignment class */
    private $edusignment;
     ...
     * @param edusign $edusignment The current edusignment
     */
    public function __construct($importid, edusign $edusignment, $encoding = 'utf-8', $separator = 'comma') {
    ...
    public function parsecsv($csvdata) {
        $this->csvreader = new csv_import_reader($this->importid, 'edusignfeedback_offline');
    ...
    public function init() {
        ...
        $strid = get_string('recordid', 'edusign');
        $strmodified = get_string('lastmodifiedgrade', 'edusign');
    ...
    public function next() {
        ...
        while ($record = $this->csvreader->next()) {
            ...
            $id = substr($idstr, strlen(get_string('hiddenuser', 'edusign')));
            if ($userid = $this->edusignment->get_user_id_for_uniqueid($id)) {
