 ...
 *
 *
 * @package   edusignfeedback_offline
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class edusign_feedback_offline extends edusign_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'edusignfeedback_offline');
    }
    ...
    /**
     * Loop through uploaded grades and update the grades for this edusignment
     *
    ...
    public function process_import_grades($draftid, $importid, $ignoremodified, $encoding = 'utf-8', $separator = 'comma') {
        ...
        // Does this edusignment use a scale?
        $scaleoptions = null;
        if ($this->edusignment->get_instance()->grade < 0) {
            if ($scale = $DB->get_record('scale', array('id' => -($this->edusignment->get_instance()->grade)))) {
                $scaleoptions = make_menu_from_list($scale->scale);
            }
        }
        // We may need to upgrade the gradebook comments after this update.
        $adminconfig = $this->edusignment->get_admin_config();
        ...
        while ($record = $gradeimporter->next()) {
            ...
            $usergrade = $this->edusignment->get_user_grade($user->id, false);
            ...
            if ($usergrade && $usergrade->grade == $record->grade) {
                ...
                $skip = true;
            } else if ($this->edusignment->grading_disabled($record->user->id)) {
            ...
            if (!$skip) {
                $grade = $this->edusignment->get_user_grade($record->user->id, true);

                $grade->grade = $record->grade;
                $grade->grader = $USER->id;
                if ($this->edusignment->update_grade($grade)) {
                ...
                foreach ($record->feedback as $feedback) {
                    ...
                    if ($newvalue != $oldvalue) {
                        ...
                        if (($plugin->get_subtype() . '_' . $plugin->get_type()) == $gradebookplugin) {
                            ...
                            $this->edusignment->update_grade($grade);
                        }
                    }
                }
            }
        ...
        $renderer = $this->edusignment->get_renderer();
        $o = '';
    ...
    public function upload_grades() {
        ...
        require_capability('mod/edusign:grade', $this->edusignment->get_context());
        require_once($CFG->dirroot . '/mod/edusign/feedback/offline/uploadgradesform.php');
        require_once($CFG->dirroot . '/mod/edusign/feedback/offline/importgradesform.php');
        require_once($CFG->dirroot . '/mod/edusign/feedback/offline/importgradeslib.php');
    ...
    public function download_grades() {
        ...
        $groupmode = groups_get_activity_groupmode($this->edusignment->get_course_module());
        // All users.
        $groupid = 0;
        $groupname = '';
        if ($groupmode) {
    ...
    public function is_enabled() {
        if ($this->enabledcache === null) {
            $gradingmanager = get_grading_manager($this->edusignment->get_context(), 'mod_edusign', 'submissions');
