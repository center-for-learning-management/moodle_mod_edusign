 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class edusignfeedback_file_import_zip_form extends moodleform implements renderable {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'uploadzip', get_string('confirmuploadzip', 'edusignfeedback_file'));
        ...
        foreach ($files as $unzippedfile) {
            ...
            if ($importer->is_valid_filename_for_import($edusignment, $unzippedfile, $participants, $user, $plugin, $filename)) {
                if ($importer->is_file_modified($edusignment, $user, $plugin, $filename, $unzippedfile)) {
        ...
        if (count($updates)) {
            ...
            $mform->addElement('html', get_string('nochanges', 'edusignfeedback_file'));
        }
