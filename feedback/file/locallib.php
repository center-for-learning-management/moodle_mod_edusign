...
class edusign_feedback_file extends edusign_feedback_plugin {
    ...
    public function is_feedback_modified(stdClass $grade, stdClass $data) {
        ...
        if (isset($filekey)) {
            ...
            $filecount = $this->count_files($grade->id, EDUSIGNFEEDBACK_FILE_FILEAREA);
    ...
    public function update_file_count($grade) {
        ...
        if ($filefeedback) {
            $filefeedback->numfiles = $this->count_files($grade->id, EDUSIGNFEEDBACK_FILE_FILEAREA);
            return $DB->update_record('edusignfeedback_file', $filefeedback);
        } else {
            $filefeedback = new stdClass();
            $filefeedback->numfiles = $this->count_files($grade->id, EDUSIGNFEEDBACK_FILE_FILEAREA);
            $filefeedback->grade = $grade->id;
            $filefeedback->edusignment = $this->edusignment->get_instance()->id;
            return $DB->insert_record('edusignfeedback_file', $filefeedback) > 0;
        }
    }
    ...
    public function upgrade(
        ...
        $filefeedback->grade = $grade->id;
        $filefeedback->edusignment = $this->edusignment->get_instance()->id;
        if (!$DB->insert_record('edusignfeedback_file', $filefeedback) > 0) {
    ...
    public function view_batch_upload_files($users) {
        ...
        foreach ($users as $userid) {
            if ($usercount >= EDUSIGNFEEDBACK_FILE_MAXSUMMARYUSERS) {
                $moreuserscount = count($users) - EDUSIGNFEEDBACK_FILE_MAXSUMMARYUSERS;
        ...
        if ($mform->is_cancelled()) {
            ...
            $o .= $this->edusignment->get_renderer()->render_footer();
        }
    ...
    public function view_upload_zip() {
        ...
        require_capability('mod/edusign:grade', $this->edusignment->get_context());
        require_once($CFG->dirroot . '/mod/edusign/feedback/file/uploadzipform.php');
        ...
        $importer = new edusignfeedback_file_zip_importer();
    ...
    /**
     * Called by the edusignment module when someone chooses something from the
