 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class edusignfeedback_file_zip_importer {

    /**
     * Is this filename valid (contains a unique participant ID) for import?
     *
     * @param edusign $edusignment - The edusignment instance
     ...
     * @param edusign_plugin $plugin - Set to the plugin that exported the file
     ...
     */
    public function is_valid_filename_for_import($edusignment, $fileinfo, $participants, & $user, & $plugin, & $filename) {
        ...
        $filename = $info[4];
        $plugin = $edusignment->get_plugin_by_type($info[2], $info[3]);
     ...
     * @param edusign_plugin $plugin The matching plugin from the filename
     ...
     */
    public function is_file_modified($edusignment, $user, $plugin, $filename, $fileinfo) {
        $sg = null;

        if ($plugin->get_subtype() == 'edusignsubmission') {
            $sg = $edusignment->get_user_submission($user->id, false);
        } else if ($plugin->get_subtype() == 'edusignfeedback') {
            $sg = $edusignment->get_user_grade($user->id, false);
        } else {
            return false;
        }
     ...
     *
     * @param int $contextid - The context id of this edusignment instance
     ...
     * @param int $contextid The context for this edusignment
     ...
     *
     * @param edusign $edusignment - The edusignment instance
     * @param edusign_feedback_file $fileplugin - The file feedback plugin
    ...
    public function import_zip_files($edusignment, $fileplugin) {
        ...
        $feedbackfilesadded = 0;
        $userswithnewfeedback = array();
        $contextid = $edusignment->get_context()->id;

        $fs = get_file_storage();
        $files = $this->get_import_files($contextid);

        $currentgroup = groups_get_activity_group($edusignment->get_course_module(), true);
        $allusers = $edusignment->list_participants($currentgroup, false);
        $participants = array();
        foreach ($allusers as $user) {
            $participants[$edusignment->get_uniqueid_for_user($user->id)] = $user;
        }
        ...
        foreach ($files as $unzippedfile) {
            ...
            if ($this->is_valid_filename_for_import($edusignment, $unzippedfile, $participants, $user, $plugin, $filename)) {
                if ($this->is_file_modified($edusignment, $user, $plugin, $filename, $unzippedfile)) {
                    $grade = $edusignment->get_user_grade($user->id, true);
                    ...
                    $edusignment->notify_grade_modified($grade);
                }
            }
        }
        ...
        $o .= $edusignrenderer->render_footer();
        return $o;
    }
