...
/**
 * This file contains the functions for edusign_plugin abstract class
 *
 *
 * @package   mod_edusign
...
/**
 * Abstract class for edusign_plugin (submission/feedback).
 *
 * @package   mod_edusign
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
abstract class edusign_plugin {
     ...
     */
    protected $edusignment;
    /** @var string $type edusignment plugin type */
    private $type = '';
    ...
    /**
     * The edusignment subtype is responsible for saving it's own settings as the database table for the
    ...
    public final function set_config($name, $value) {
        ...
        if ($current) {
            $current->value = $value;
            return $DB->update_record('edusign_plugin_config', $current);
        } else {
            $setting = new stdClass();
            $setting->edusignment = $this->edusignment->get_instance()->id;
            $setting->subtype = $this->get_subtype();
            $setting->plugin = $this->get_type();
            $setting->name = $name;
            $setting->value = $value;

            return $DB->insert_record('edusign_plugin_config', $setting) > 0;
        }
    }
    ...
    public final function get_config($setting = null) {
        ...
        if ($setting) {
            if (!$this->edusignment->has_instance()) {
                return false;
            }
     ...
     * and version.
     *
     * @param string $type The old edusignment subtype
     * @param int $version The old edusignment version
    ...
    /**
     * Upgrade the settings from the old edusignment to the new one
     *
     * @param context $oldcontext The context for the old edusignment module
     ...
     * @param stdClass $submissionorgrade edusign_submission or edusign_grade The new submission or grade
     * @param string $log Record upgrade messages in the log
    ...
    /**
     * The edusignment has been deleted - remove the plugin specific data
     *
     * @return bool
     */
        ...
        if ($this->get_subtype() == 'edusignsubmission') {
            if ($itemid) {
                ...
                if (!empty($record->userid)) {
                    if (!$edusignment->can_view_submission($record->userid)) {
                        return null;
                    }

                    // We only report write access for teachers.
                    $writeaccess = $edusignment->can_grade() && $edusignment->can_edit_submission($record->userid);
                } else {
                    // Must be a team submission with a group.
                    if (!$edusignment->can_view_group_submission($record->groupid)) {
                        return null;
                    }
                    // We only report write access for teachers.
                    $writeaccess = $edusignment->can_grade() && $edusignment->can_edit_group_submission($record->groupid);
                }
            }
        } else {
    ...
    /**
     * This allows a plugin to render a page in the context of the edusignment
     *
     * If the plugin creates a link to the edusignment view.php page with
     * The following required parameters:
     *      id=coursemoduleid
     *      plugin=type
     *      pluginsubtype=edusignfeedback|edusignsubmission
     ...
     * right below the activity's "intro" section on the main edusignment page.
     *
     * @return string
     */
     ...
     * enabled/disabled per edusignment instance.
     *
     * @return bool
     */
